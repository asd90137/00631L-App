...
try:
    ...
    st.sidebar.error("❌ 帳本連結失敗，請檢查權限設定。")
...
if st.session_state.analyzed:
    ...
    with tab1:
        ...
        days_tw = (datetime.today() - temp_tw['成交日期'].min()).days if not temp_tw.empty else 1
        ...
        c11.metric("台股佔持股比", f"{(cur_val_tw/(cur_val_tw+total_us_val_twd)*100):.1f}%", "佔兩具引擎比例")

        with st.expander("📜 逐筆投資戰績表 (含損益與年化)", expanded=False):
            ...
            for _, r in buy_tw.sort_values('成交日期', ascending=False).iterrows():
                ...
                recs_tw.append({'日期': r['成交日期'].strftime('%Y-%m-%d'), '買價': f"{adj_p:.2f}", '張數': f"{adj_s/1000:.1f}", '損益額': f"{l_pnl:+,.0f}", '年化': f"{l_ann:+.1f}%", '總報酬': f"{l_roi*100:+.1f}%"})
            st.dataframe(pd.DataFrame(recs_tw), use_container_width=True, hide_index=True)

        with st.expander("🛒 新增台股交易紀錄 (網頁即時上傳)", expanded=False):
            col_a, col_b = st.columns(2); tw_date = col_a.date_input("成交日期", key='tw_d'); tw_type = col_b.selectbox("交易類型", ["現股買入", "現股賣出"], key='tw_t')
            col_c, col_d = st.columns(2); tw_price = col_c.number_input("成交價格", key='tw_p'); tw_shares = col_d.number_input("股數", key='tw_s')
            tw_fee = st.number_input("手續費", value=0, key='tw_f')
            if st.button("🚀 寫入台股試算表", use_container_width=True):
                new_row = pd.DataFrame([{"成交日期": tw_date.strftime("%Y-%m-%d"), "交易類型": tw_type, "成交價格": tw_price, "庫存股數": tw_shares, "手續費": tw_fee, "持有成本": (tw_price*tw_shares)+tw_fee}])
                updated_tw = pd.concat([df_tw, new_row], ignore_index=True)
                conn.update(spreadsheet=SHEET_TW, data=updated_tw)
                st.success("✅ 台股紀錄已成功上傳！")

        st.subheader("🌐 戰術圖表分析 (已還原股價)")
        ...
        fig1 = go.Figure(); fig1.add_trace(go.Scatter(x=adj_h.index, y=adj_h.values, name="還原價", line=dict(color='#E71D36')))
    ...
    with tab2:
        ...
        st.info(f"💡 **SOXL 壓力預演：** 若 SOXX 跌回 100DMA，SOXL 預計來到 **${soxl_pred:.2f}** (距現值 {((soxl_pred/soxl_c-1)*100):.1f}%)")
        ...
        min_date_us = min([v['first_date'] for v in us_live.values() if not pd.isna(v['first_date'])])
        ...
        u3.metric("未實現損益", f"{(total_us_val_usd-total_us_cost_usd):+,.0f}", f"{us_roi*100:+.2f}%")
        u4.metric("年化報酬率", f"{ann_roi_us:+.2f}%")

        st.subheader("📦 個股明細 (不折疊)")
        us_table = []
        for t, info in us_live.items():
            ...
            us_table.append({'代號': t, '股數': f"{info['shares']:,.0f}", '均價': f"{avg:.2f}", '成本': f"{info['cost']:,.0f}", '昨日': f"{info['yest']:.2f}", '現價': f"{info['curr']:.2f}", '今日損益': f"{(info['curr']/info['yest']-1)*100:+.2f}%", '總損益': f"{l_roi*100:+.1f}%", '年化': f"{l_ann:+.1f}%"})
        ...
        with st.expander("🛒 新增美股交易紀錄 (網頁即時上傳)", expanded=False):
            col_ua, col_ub = st.columns(2); us_date = col_ua.date_input("成交日期", key='us_d'); us_ticker = col_ub.text_input("股票代號 (如: SOXL)", key='us_t')
            col_uc, col_ud = st.columns(2); us_price = col_uc.number_input("成交價格", key='us_p'); us_shares = col_ud.number_input("股數", key='us_s')
            us_fee = st.number_input("手續費 (USD)", value=0.0, key='us_f')
            if st.button("🚀 寫入美股試算表", use_container_width=True):
                new_row_us = pd.DataFrame([{"成交日期": us_date.strftime("%Y-%m-%d"), "股票代號": us_ticker, "交易類型": "現股買入", "成交價格": us_price, "庫存股數": us_shares, "手續費": us_fee, "持有成本": (us_price*us_shares)+us_fee}])
                updated_us = pd.concat([df_us, new_row_us], ignore_index=True)
                conn.update(spreadsheet=SHEET_US, data=updated_us)
                st.success("✅ 美股紀錄已成功上傳！")
    ...
    with tab3:
        ...
        st.markdown(f"""
        | 戰區 | 曝險金額 (台幣) | 淨資產 (FC) | 實際曝險度 | 備註 (美金原值/匯率) |
        | :--- | :--- | :--- | :--- | :--- |
        | 🇹🇼 台股 | NT$ {exp_tw/10000:,.0f} 萬 | NT$ {(cur_val_tw+cash/2-loan1/2-loan2/2)/10000:,.0f} 萬 | **{(exp_tw/FC*100):.1f}%** | - |
        | 🇺🇸 美股 | NT$ {exp_us/10000:,.0f} 萬 | NT$ {(total_us_val_twd+cash/2-loan1/2-loan2/2)/10000:,.0f} 萬 | **{(exp_us/FC*100):.1f}%** | US ${exp_us/usd_twd:,.0f} (匯率:{usd_twd}) |
        ...
        c_tgt, c_act = st.columns(2); c_tgt.metric("🎯 生命周期目標曝險度", f"{target_E:.1f}%"); c_act.metric("🔥 現在總曝險度", f"{(exp_total:=(exp_tw+exp_us)/FC*100):.1f}%", f"差距: {(exp_total - target_E):+.1f}%")
...
st.caption("📱 提示：數字顯示已全面轉換為「萬」，避免螢幕過窄造成數據截斷。台股分割日期 3/23 已設定自動還原。")
