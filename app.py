...
if st.session_state.analyzed:
    ...
    with tab1:
        ...
        with col_p:
            fig_p = go.Figure(data=[go.Pie(labels=['00631L 市值', '可用現金'], values=[cur_val_tw, cash], hole=.4, marker_colors=['#E71D36', '#2EC4B6'])])
            fig_p.update_layout(height=250, margin=dict(l=0,r=0,t=30,b=0), title="資產配置比例 (不含負債)")
        ...
        with st.expander("📜 逐筆投資戰績表", expanded=False):
            ...
            for _, r in buy_tw.sort_values('成交日期', ascending=False).iterrows():
                ...
                recs_tw.append({'日期': r['成交日期'].strftime('%Y-%m-%d'), '買價': f"{adj_p:.2f}", '張數': f"{adj_s/1000:.1f}", '今日損益': f"{(p_tw_curr-p_tw_yest)*adj_s:+,.0f}", '總損益': f"{l_pnl:+,.0f}", '年化': f"{l_ann:+.1f}%", '報酬': f"{l_roi*100:+.1f}%"})
        ...
        with st.expander("🛒 新增台股交易紀錄", expanded=False):
            ...
            if st.button("🚀 寫入台股試算表", use_container_width=True) and tw_shares > 0:
                ...
                st.success("✅ 台股紀錄上傳成功！請重新啟動掃描。")

        st.subheader("🌐 戰術圖表分析 (還原股價)")
        ...
        rp = adj_h[adj_h.index >= min_date_tw]
        ...
        mx, mi = rp.max(), rp.min(); fig1.add_annotation(x=rp.idxmax(), y=mx, text=f"高:{mx:.2f}", showarrow=True); fig1.add_annotation(x=rp.idxmin(), y=mi, text=f"低:{mi:.2f}", showarrow=True)
        ...
        if not temp_tw.empty:
            ...
            fig3 = go.Figure(); fig3.add_trace(go.Scatter(x=rp.index, y=dp, name='損益%', line=dict(color='#247BA0')))
    ...
    with tab2:
        ...
        for t, info in us_live.items():
            ...
            us_table.append({'代號': t, '股數': f"{info['shares']:,.0f}", '均價': f"{avg:.2f}", '成本': f"{info['cost']:,.0f}", '現價': f"{info['curr']:.2f}", '今日': f"{(info['curr']/info['yest']-1)*100:+.2f}%", '總損益': f"{l_roi*100:+.1f}%", '年化': f"{l_ann:+.1f}%"})
        ...
        with st.expander("🛒 新增美股交易紀錄", expanded=False):
            ...
            c_ue, c_uf = st.columns(2); us_sh = c_ue.number_input("股數", key='u_s'); us_fe = c_uf.number_input("手續費(USD)", key='u_f')
            st.info(f"💰 預估美股成本：**US$ {(us_pr*us_sh)+us_fe:,.2f}**")
            if st.button("🚀 寫入美股試算表", use_container_width=True) and us_sh > 0:
                new_us = pd.DataFrame([{"成交日期": us_date.strftime("%Y-%m-%d"), "股票代號": us_tk.upper(), "交易類型": us_type, "成交價格": us_pr, "庫存股數": us_sh, "手續費": us_fe, "持有成本": (us_pr*us_sh)+us_fe}])
    ...
    with tab3:
        ...
        | 🇺🇸 美股 | NT$ {exp_us/10000:,.0f} 萬 | NT$ {(total_us_val_twd+cash/2-(loan1+loan2)/2)/10000:,.0f} 萬 | **{(exp_us/FC*100):.1f}%** | 曝險: **${exp_us_usd:,.0f}** <br> 淨值: **${total_us_val_usd:,.0f}** |
        ...
        ca1, ca2, ca3 = st.columns(3); ca1.metric("屆時滾出資產", f"NT$ {f_a/10000:,.0f} 萬"); ca2.metric("未來每月可領", f"NT$ {m_a:,.0f}"); ca3.metric("約等同現在月薪", f"NT$ {m_a_now:,.0f}")
...
st.caption("📱 提示：台美股記帳功能已全面修復。圖表 C 損益軌跡與極值標示已回歸。")
