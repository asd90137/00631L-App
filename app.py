...
if st.session_state.analyzed:
    ...
    with tab2:
        ...
        * **🅿️ 資金停泊：** 閒置資金請優先停泊於 **1、2、3 個月期美國國債**，等待大跌機會。
        ...
        # --- 新增的指標計算 ---
        soxl_yest = us_live.get('SOXL', {}).get('yest', 0)
        today_ret_pct = (soxl_c / soxl_yest - 1) * 100 if soxl_yest > 0 else 0
        cur_ret_pct = (soxl_c / avg_p - 1) * 100 if avg_p > 0 else 0
        expected_profit = (tp_price - avg_p) * tot_s if tp_price > 0 else 0
        ...
        c_g1.metric("目前進度", f"{cur_tranche_name}", f"庫存 {tot_s:,.0f} 股" if tot_s > 0 else "")
        c_g2.metric("目前股價", f"${soxl_c:.2f}", f"今日報酬 {today_ret_pct:+.2f}%")
        c_g3.metric("平均股價", f"${avg_p:.2f}", f"目前報酬 {cur_ret_pct:+.2f}%")
        
        # 目標停利：將預估賺的錢放進標題裡
        c_g4.metric(f"目標停利 (預估賺 ${expected_profit:,.0f})", f"${tp_price:.2f}", f"差距 {tp_dist:+.2f}%" if soxl_c > 0 and tp_price > 0 else "N/A")
...
st.caption("📱 提示：V10.2 SOXL網格全數據版，已實裝目前股價、今日本日報酬率與預估停利金額。")
