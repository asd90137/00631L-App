...
class CONFIG:
    ...
    SHEET_TW = "[https://docs.google.com/spreadsheets/d/1yYs-JIW4-8jr8EoyyWlydNrE5Gtd_frWdlMQVdn1VYk/edit?usp=sharing](https://docs.google.com/spreadsheets/d/1yYs-JIW4-8jr8EoyyWlydNrE5Gtd_frWdlMQVdn1VYk/edit?usp=sharing)"
    SHEET_US = "[https://docs.google.com/spreadsheets/d/1-NPhyuRNWSarFPdgHjUkB9J3smSbn3u3fjUbMhMVyfI/edit?usp=sharing](https://docs.google.com/spreadsheets/d/1-NPhyuRNWSarFPdgHjUkB9J3smSbn3u3fjUbMhMVyfI/edit?usp=sharing)"
...
def fetch_us_price(ticker: str) -> dict:
    ...
    try:
        ...
        if not hist.empty:
            ...
            age_min = (dt_mod.datetime.now(et_tz) - last_time).total_seconds() / 60

            return dict(curr=curr, prev=prev, session=session,
                        source="🟢 yfinance ", time_str=time_str, age_min=age_min)
    except Exception as e:
        pass
...
def render_tab_lifecycle(port: dict, base_m: float, hc_years: int, target_k: float,
    ...
    |💵 美股|NT$ {exp_us/10000:,.0f} 萬<br/><span style="font-size: 0.85em; color: gray;"> {port['exp_us_usd']:,.0f}</span>|NT$ {fc_us/10000:,.0f} 萬<br/><span style="font-size: 0.85em; color: gray;">  {port['fc_us_usd']:,.0f}</span>|**{pct_us:.1f}%**|
