...
def next_first_wednesday(from_date) -> datetime.date:
today = from_date
...
def sniper_signal(daily_pct: float) -> tuple:
...
try:
    ...
    try:
        ...
        dt = (datetime.fromtimestamp(ts, tz=CONFIG.TW_TZ) if isinstance(ts, (int, float))
              else ts.astimezone(CONFIG.TW_TZ))
        age_min  = (datetime.now(CONFIG.TW_TZ) - dt).total_seconds() / 60
...
try:
    ...
    return dict(curr=1.0, prev=1.0, source="❌ 完全失敗",
                time_str="N/A", age_min=99999, session="❓")
```

def _parse_fugle_time(raw_t) -> tuple:
try:
if isinstance(raw_t, (int, float)):
unit = “us” if raw_t > 1e14 else (“ms” if raw_t > 1e11 else “s”)
dt = pd.to_datetime(raw_t, unit=unit, utc=True).astimezone(CONFIG.TW_TZ)
else:
dt = pd.to_datetime(raw_t)
dt = dt.tz_localize(“Asia/Taipei”) if dt.tzinfo is None else dt.astimezone(CONFIG.TW_TZ)
age_min = (datetime.now(CONFIG.TW_TZ) - dt).total_seconds() / 60
...
rem = ((principal * ((1 + r) ** N - (1 + r) ** passed) / ((1 + r) ** N - 1))
if r > 0 else max(0, principal - pmt * passed))
...
df = df_raw.copy()
df["_K"] = pd.to_numeric(df[col_k].astype(str).str.replace(r"[^\d.]", "", regex=True),
                          errors="coerce").fillna(0)

if col_d:
    df["_D"] = pd.to_numeric(df[col_d].astype(str).str.replace(r"[^\d.]", "", regex=True),
                              errors="coerce").fillna(0)
...
def parse_cash_parking(df_raw: pd.DataFrame) -> list:
result = []
if df_raw.empty:
return result

```
col_type = next((c for c in df_raw.columns
                 if "停泊" in str(c) or ("類型" in str(c) and "停" in str(c))), None)
col_amt  = next((c for c in df_raw.columns
                 if ("停泊" in str(c) and "金額" in str(c)) or "金額" in str(c)), None)
col_mat  = next((c for c in df_raw.columns if "到期" in str(c)), None)
col_note = next((c for c in df_raw.columns
                 if "備註" in str(c) and col_type and c != col_type), None)
...
for _, row in df_raw.iterrows():
    ...
    result.append(dict(type=t, amount_usd=amt,
...
tw_price: dict = None):
if tw_price:
render_price_freshness(tw_price[“source”], tw_price[“time_str”],
tw_price[“age_min”], tw_price.get(“session”, “”))
...
with col3:
    ...
    if final_amt > 0:
        ...
        st.metric("換算購買股數",
                  f"約 {(final_amt/p_tw_curr):,.0f} 股" if p_tw_curr > 0 else "0 股")
    else:
...
st.divider()
st.link_button("🛒 新增台股交易紀錄 (Google Sheets)", CONFIG.SHEET_TW,
...
```
    ...
    fig1 = go.Figure()
    fig1.add_trace(go.Scatter(x=rp.index, y=rp.values, name="還原價",
    ...
    if not bc.empty:
        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(x=bc.index, y=bc.values, name="乖離%",
                                  line=dict(color="#F4A261")))
        for v, c, t in [(-5, "gray", "標準(-5)"), (-10, "orange", "恐慌(-10)"),
                         (-15, "red", "重壓(-15)")]:
            fig2.add_hline(y=v, line_dash="dot", line_color=c, annotation_text=t)
        bx, bi, bl = bc.max(), bc.min(), bc.iloc[-1]
        fig2.add_hrect(y0=0, y1=max(bx * 1.2, 10),
                       fillcolor="green", opacity=0.1, layer="below", line_width=0)
        fig2.add_hrect(y0=min(bi * 1.2, -20), y1=0,
                       fillcolor="red", opacity=0.1, layer="below", line_width=0)
        fig2.add_annotation(x=bc.idxmax(), y=bx, text=f"最高:{bx:.1f}%",
                             showarrow=True, ay=-30)
        fig2.add_annotation(x=bc.idxmin(), y=bi, text=f"最低:{bi:.1f}%",
                             showarrow=True, ay=30)
        fig2.add_annotation(x=bc.index[-1], y=bl, text=f"最新:{bl:.1f}%",
    ...
    buy_df = tw_trade.get("raw_buys", pd.DataFrame())
    if not buy_df.empty:
        th = (buy_df.groupby("成交日期")[["庫存股數", "持有成本"]]
                    .sum().reindex(rp.index).fillna(0))
        ds = th["庫存股數"].cumsum()
        dc = th["持有成本"].cumsum()
        ...
        if not dp_s.empty:
            ...
            fig3 = go.Figure()
            fig3.add_trace(go.Scatter(x=dp_s.index, y=dp_s.values,
                                      line=dict(color="#247BA0")))
            fig3.add_hrect(y0=0, y1=max(px * 1.2, 10),
                           fillcolor="green", opacity=0.1, layer="below", line_width=0)
            fig3.add_hrect(y0=min(pi * 1.2, -10), y1=0,
                           fillcolor="red", opacity=0.1, layer="below", line_width=0)
            fig3.add_annotation(x=dp_s.idxmax(), y=px, text=f"最高:{px:.1f}%",
                                 showarrow=True, ay=-30)
            fig3.add_annotation(x=dp_s.idxmin(), y=pi, text=f"最低:{pi:.1f}%",
                                 showarrow=True, ay=30)
            fig3.add_annotation(x=dp_s.index[-1], y=pl, text=f"最新:{pl:.1f}%",
                                 showarrow=True, ax=40)
        ...
        fig4 = go.Figure()
        fig4.add_trace(go.Scatter(x=cc_m.index, y=cc_m.values, name="累積成本",
                                  line=dict(color="#888888", width=2)))
        fig4.add_trace(go.Scatter(x=mv_m.index, y=mv_m.values, name="市值",
                                  line=dict(color="#2EC4B6", width=2.5)))
        fig4.add_annotation(x=mv_m.idxmax(), y=mv_m.max(),
                             text=f"最高:{mv_m.max():.2f}M", showarrow=True, ay=-30)
        fig4.add_annotation(x=mv_m.index[-1], y=mv_m.iloc[-1],
                             text=f"最新:{mv_m.iloc[-1]:.2f}M", showarrow=True, ax=40)
        fig4.add_annotation(x=cc_m.index[-1], y=cc_m.iloc[-1],
                             text=f"成本:{cc_m.iloc[-1]:.2f}M", showarrow=True, ay=30, ax=40)
        st.plotly_chart(fig4, use_container_width=True)
        pnl_color = "#2EC4B6" if last_pnl >= 0 else "#E71D36"
        st.markdown(
            f"<p style='color:{pnl_color}; font-size:16px; margin:0'>"
            f"目前損益：{sign}NT$ {last_pnl:.2f}M</p>",
...
c3.metric(f"平均股價 ({g['total_shares']:,.0f} 股)", f"${g['avg_price']:.2f}",
          f"{cur_roi:+.2f}%")
c4.metric(f"目標停利 ({g['tp_pct']:.0f}%,  預估+${est_profit:,.0f})",
          f"${g['tp_price']:.2f}",
          f"差距 {tp_dist:+.2f}%" if soxl_curr > 0 and g["tp_price"] > 0 else "N/A")
if g["next_add_price"] > 0:
    c5.metric(f"加碼股價 ({g['next_add_shares']:,.0f} 股)",
              f"${g['next_add_price']:.2f}",
              f"差距 {add_dist:+.2f}%" if soxl_curr > 0 else "N/A")
else:
...
st.divider()
st.link_button("🛒 新增美股交易紀錄 (Google Sheets)", CONFIG.SHEET_US,
               use_container_width=True)
```

def render_tab_lifecycle(port: dict, base_m: float, hc_years: int, target_k: float,
target_monthly_now: float, inflation_rate: float,
withdrawal_rate: float, usd_twd: float):
...
|💵 美股|NT$ {exp_us/10000:,.0f} 萬     |NT$ {fc_us/10000:,.0f} 萬       |**{pct_us:.1f}%** |
...
with st.expander("🛬 降落時程推演表 (Glide Path)", expanded=False):
    ...
    for y in range(hc_years + 1):
        ...
        gp.append({"年": f"第 {y} 年", "預估資產(萬)": f"{cf/10000:,.0f}",
                    "應有曝險": f"{e_g:.1f}%"})
...
def calc_tranche(date_str, share_ratio):
...
df_schedule = pd.DataFrame([
    {"解鎖梯次": "第一梯次 (50%)", "預計月份": t1_month, "距離天數": f"約 {t1_days} 天",
     "對應張數": f"{t1_s/1000:.0f} 張", "目前現值": f"{t1_v:,.0f} 元",
     "未實現損益": f"{t1_p:+,.0f} 元"},
    {"解鎖梯次": "第二梯次 (25%)", "預計月份": t2_month, "距離天數": f"約 {t2_days} 天",
     "對應張數": f"{t2_s/1000:.0f} 張", "目前現值": f"{t2_v:,.0f} 元",
     "未實現損益": f"{t2_p:+,.0f} 元"},
    {"解鎖梯次": "第三梯次 (25%)", "預計月份": t3_month, "距離天數": f"約 {t3_days} 天",
     "對應張數": f"{t3_s/1000:.0f} 張", "目前現值": f"{t3_v:,.0f} 元",
     "未實現損益": f"{t3_p:+,.0f} 元"},
])
...
1. 越接近解鎖日 (2027/04/16)，鎖定價格的急迫性越高。
   “””)
...
try:
    ...
    st.sidebar.warning(f"⚠️ Google Sheets 連線失敗: {e}")
