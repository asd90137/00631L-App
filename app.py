...
st.write("在這裡輸入的資料，將會直接同步寫入 Google 試算表！")
...
with st.form("add_trade_form"):
    ...
    submitted = st.form_submit_button("🚀 送出並寫入雲端", use_container_width=True)
    
    if submitted:
        if not df_trades_raw.empty:
            # 計算持有成本 (買進 = 價格*股數+手續費，賣出請直接輸入原本試算表的習慣)
            cost = (trade_price * trade_shares) + trade_fee
            ...
            try:
                ...
                st.cache_data.clear() # 清除暫存，確保下次抓到最新資料
                st.success("✅ 交易紀錄已成功寫入 Google 試算表！請重新整理網頁即可看到最新庫存。")
            except Exception as e:
